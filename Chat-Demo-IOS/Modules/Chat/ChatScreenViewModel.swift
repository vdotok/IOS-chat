...
protocol ChatScreenViewModel: ChatScreenViewModelInput {
    ...
    var messages: [BaseMessage] {get set}
    ...
    func itemAt(row: Int) -> (BaseMessage, CellType)
    func receivedMessage(userInfo: [String: AnyObject])
    func dispatchPackage(start: Bool)
    func sendSeenMessage(message: BaseMessage, row: Int)
...
class ChatScreenViewModelImpl: ChatScreenViewModel, ChatScreenViewModelInput {
    ...
    init(router: ChatScreenRouter, client: ChatClient, group: Group, user: UserResponse, messages: [BaseMessage]) {
        self.router = router
        self.mqtt = client
        self.group = group
        self.user = user
...
extension ChatScreenViewModelImpl {
    func itemAt(row: Int) -> (BaseMessage,CellType) {
        ...
        if messages[row].from != user.refID && messages[row].content == "" {
...
//            }
        ...
        } else if messages[row].from == user.refID && messages[row].content == ""  {
        ...
        if messages[row].from == user.refID  {
